...
The subtile crop origin within a stored tile uses the **precise fractional** subtile size `round(q·Dim/2ⁱ)`, not `q·(Dim>>i)` (v0.59.1, #68 follow-up). A stored tile packs 2ⁱ subtiles per axis across its full `TileW`/`TileH`; flooring the per-subtile size accumulates a crop-origin error of up to `(2ⁱ−1)·frac` px when the dimension is not a multiple of 2ⁱ. Legacy iScan tiles are **non-square** `1024×1360`: `TileW=1024` is a power of two (exact at every level), but `1360` is only divisible by 16, so `1360/32 = 42.5` first loses precision at **L5**, drifting the vertical crop up to ~15 px at the bottom of each stored tile — a deep-zoom cross-level "drift" (L1–L4 are exact since `1360 = 16·85`). DP tiles are square `1024×1024` (power of two) so DP is unaffected. Cross-level registration is now ≤ 1 px at every level (`TestReducedDeepLevelRegistration`).
