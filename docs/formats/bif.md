...
**Consumer contract:** if `Level.Overlapping`, do **not** iterate `Grid` as if it tiled `Size`; route pixel reassembly through the region API (gate any verbatim per-tile-copy fast path on `!Overlapping`). `Overlapping` is `false` for every non-BIF format. For BIF it is `true` on L0 **and** the reduced levels whenever L0 was overlap-compacted — both DP (v0.55.0, #83) and legacy iScan (v0.55.0, #80). Reduced levels carry the frame overlap (DP: `~overlap/2ⁱ` residual at the frame-join seams; legacy: dense ~11%); their stitch layout is **downsampled from L0's** (reduced tile `(col,row)` inherits L0 frame `(col<<i, row<<i)`'s compacted origin scaled by `1/2ⁱ`), and their `Size` is the L0 stitched hull floor-halved (matching bio-formats for DP / openslide for legacy within rounding). `Level.TileOverlap` carries the L0 overlap magnitude; reduced levels report `TileOverlap=(0,0)` (the residual is non-uniform, so the region layout — not a single magnitude — is authoritative; gate on `Overlapping`, not `TileOverlap`).
