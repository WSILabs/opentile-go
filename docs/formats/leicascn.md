...
2. **Geometry pinning + cross-backing parity** (`tests/parity/leicascn_geometry_test.go`) — all 3 fixtures, no build tag, runs in `make test`. Pins per-level Size / TileSize / Grid / Compression, AssociatedImage types + sizes, Image.SizeC + ChannelName, distinct-bytes-per-channel sanity (3 distinct hashes for Fluorescence), Metadata fields (ScannerModel, Barcode, Regions count, per-Auxiliary IlluminationSource), and tile-byte equality across mmap / pread backings.
