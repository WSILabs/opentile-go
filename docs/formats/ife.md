...
| Associated images (IMAGE_ARRAY) | ✅ | `s.AssociatedImages()` returns label / overview / thumbnail / macro / map / probability with normalised type names (v0.15+). Unknown titles surface lowercased. Per-image `Compression()` is JPEG (encoding=2) or AVIF (encoding=3); PNG (encoding=1) is reported as `CompressionUnknown` for now (passthrough — consumer can sniff the PNG signature). Cervix has one 1920×1337 JPEG thumbnail |
