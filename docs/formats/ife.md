...
**Non-conformant override.** The local `cervix_2x_jpeg.iris` fixture is the *only* file in the public corpus that carries header resolution values, and it is a **miscomputed local re-encode** (codec `2025.2.0`): its header `magnification = 0.625` and `micronsPerPixel = 16.835` were computed for a 4-layer ×1/4/16/64 pyramid (`max_scale = 64` → `0.625 × 64 = 40×`, `16.835 / 64 = 0.263 µm/px`), but the file was re-laddered to 9 layers ×1…×256 (`max_scale = 256`) without recomputing them. Applying the convention at its *actual* `max_scale = 256` would yield `0.625 × 256 = 160×` — an impossible objective — proving the file, not the convention, is wrong. To keep that fixture reporting sensible values, when the passed-through `ATTRIBUTES` carry the source scanner's authoritative L0 resolution — discrete `aperio.AppMag` / `aperio.MPP`, or the Aperio `ImageDescription` banner (`…|AppMag = 40|MPP = 0.262968|…`) — those **override** the convention (per-field, independently). Conformant files need no override. This is why the cervix fixture reports `Magnification = 40` / `MPP = 0.262968` (from `aperio.*`) while `MagnificationFromHeader = 0.625` / `MPPFromHeader = 16.835` preserve the raw header.
...
- **Header magnification/MPP are scale-relative** (#81, v0.54.0 → corrected v0.54.1): the v0.54.0 fix mis-framed the GT450 cervix header as "an encoder stamped a downsampled level's value" and preferred `aperio.*` as a special case. The actual Iris convention is that `magnification` is a coefficient (`objective = magnification × max_scale`) and `micronsPerPixel` is anchored at the lowest-resolution layer (`MPP_finest = micronsPerPixel / max_scale`) — confirmed against the Iris-Codec **encoder source** (`Iris-Codec/src/IrisCodecEncoder.cpp`), which writes `micronsPerPixel = MPP_finest × max_scale` and `magnification = objective / max_scale`. (An earlier draft of this doc claimed the canonical `425248_JPEG.iris` reference verified the numbers — it does **not**: that file leaves both header fields `0.0`/unset. The `0.625` / `16.835` values are the local miscomputed cervix re-encode, not the reference.) v0.54.1 reads the convention (`× / ÷ max_scale`) as the primary path; the `aperio.*` override now handles only the non-conformant local cervix fixture. See [Resolution convention](#resolution-convention--header-values-are-scale-relative-v0541-81).
