...
func TestJPEGXLProbe(t *testing.T) {
	...
	if !ok {
		t.Fatal("jpegxl factory does not implement decoder.Prober")
	}
	...
	if ci.Components != 3 || ci.BitDepth != 8 || ci.Lossless != decoder.LosslessUnknown ||
		...
		t.Errorf("jxl probe = %+v, want comps=3 depth=8 lossless=unknown RGB raw", ci)
	}
