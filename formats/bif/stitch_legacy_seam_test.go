...
// TestLegacyReducedSeamContinuity is the headline placement-fidelity gate for
// #80: it applies the same stitch-vs-naive overlap-band methodology as
// TestLegacySeamContinuity, but to the REDUCED level L1 — proving the
// downsample-L0 layout removes the dense legacy frame overlap from reduced
// levels too. Reduced levels carry no joints, so pairs are found directly from
// the compacted L1 layout (adjacent columns whose placement overlaps), and the
// band is measured at the layout's actual placement. A correct compaction makes
// A's right band match B's left band (stitch) far better than B's right band
// (naive control).
func TestLegacyReducedSeamContinuity(t *testing.T) {
	// absBound is the firm misplacement gate: a wrong reduced-level placement
	// (off by a tile / wrong per-column overlap) compares unrelated content and
	// runs 50–100+. Observed OS-1 L1: stitchMAD≈19.7. The ratio gate is looser
	// than L0's 0.6 because reduced tiles are the scanner's downsample (more
	// JPEG/filter noise) and the >>i origin rounding adds ≤1px registration drift
	// — so the band is intrinsically noisier than L0's joint-exact placement,
	// while still aligning real content far better than the naive no-overlap
	// control (OS-1 L1: 19.7 vs 31.1).
	const absBound = 30.0
	const ratioBound = 0.8
	for _, name := range []string{"OS-1"} { // OS-1 has the densest live joint graph
		t.Run(name, func(t *testing.T) {
			tl := openLegacyTiler(t, name)
			if len(tl.levelImpls) < 2 {
				t.Skip("no reduced level")
			}
			l1 := tl.levelImpls[1]
			if !l1.overlapping || l1.layout == nil {
				t.Fatalf("%s L1 not stitched (overlapping=%v) — #80 not engaged", name, l1.overlapping)
			}
			tileW := l1.tileSize.W
			...
			var sumStitch, sumNaive float64
			var n int
			// Interior sweep; the per-pair overlap comes from the compacted layout.
			for row := l1.grid.H / 4; row < 3*l1.grid.H/4 && n < seamSampleCap; row++ {
				for col := l1.grid.W / 4; col+1 < 3*l1.grid.W/4 && n < seamSampleCap; col++ {
					ax, _, aok := l1.layout.TileOrigin(col, row)
					bx, _, bok := l1.layout.TileOrigin(col+1, row)
					if !aok || !bok {
						continue
					}
					ov := tileW - (bx - ax)
					...
					a, ok1 := decodeTile(col, row)
					if !ok1 {
						continue
					}
					b, ok2 := decodeTile(col+1, row)
					if !ok2 {
						continue
					}
					...
					sumStitch += bandMAD(a, b, a.Width-ov, 0, ov)
					sumNaive += bandMAD(a, b, a.Width-ov, b.Width-ov, ov)
					n++
				}
			}
			if n == 0 {
				t.Skip("no decodable overlapping L1 pairs (nocgo or no texture)")
			}
			meanStitch := sumStitch / float64(n)
			meanNaive := sumNaive / float64(n)
			t.Logf("%s L1: n=%d stitchMAD=%.2f naiveMAD=%.2f", name, n, meanStitch, meanNaive)
			if meanStitch >= absBound {
				t.Errorf("%s L1: stitchMAD = %.2f, want < %.1f (reduced-level overlap bands don't match — placement wrong?)", name, meanStitch, absBound)
			}
			if meanStitch >= ratioBound*meanNaive {
				t.Errorf("%s L1: stitchMAD = %.2f, want < %.2f × naiveMAD %.2f — reduced stitch not aligning content", name, meanStitch, ratioBound, meanNaive)
			}
		})
	}
}
