...
// AssociatedSource is the on-disk encoded form of an associated image: the
...
// associatedSourcer is the optional capability implemented by associated
// images that can describe their faithful on-disk source form. It is NOT part
// of the AssociatedImage interface (no breaking change); AssociatedSourceOf
// discovers it by type assertion.
type associatedSourcer interface {
	AssociatedSource() (AssociatedSource, bool)
}

// AssociatedSourceOf returns the encoded source strips + TIFF tags for
...
func (s *Slide) AssociatedSourceOf(a AssociatedImage) (AssociatedSource, bool) {
	if p, ok := a.(associatedSourcer); ok {
		return p.AssociatedSource()
	}
	return AssociatedSource{}, false
}
