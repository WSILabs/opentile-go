...
- **API additions:** `opentile.FormatGenericTIFF` Format constant (`"generic-tiff"`); `opentile.CompressionDeflate` Compression enum value; `formats/generictiff` package (Factory + Tiler + Level + AssociatedImage + Metadata); `internal/tiff.ClassifyPyramid` + `PyramidLevelInfo` (exported for reuse); new `"associated"` AssociatedImage Kind value as the heuristic-classifier fallback (documented in `image.go`'s interface docstring).
...
- **Format doc:** `docs/formats/generictiff.md`
